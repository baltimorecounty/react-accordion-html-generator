...
class App extends Component {
    constructor(props) {
        ...
        this.handleAccordionTitleChange = this.handleAccordionTitleChange.bind(
            this
        );
        this.handleHeadingTypeChange = this.handleHeadingTypeChange.bind(this);
        this.generateHtmlFromTemplate = this.generateHtmlFromTemplate.bind(
            this
        );
    }
    ...
    handleAccordionTitleChange(accordionTitle) {
        this.setState({
            accordionTitleForIds: this.cleanStringForId(accordionTitle)
        });
    }

    handleGenerateCodeClick() {
        const output = this.state.accordionatorHtmlOutput.map(
            this.generateHtmlFromTemplate
        );

        const code = `<!-- BEGIN ${
            this.state.accordionTitleForIds
        } ACCORDION -->
		<div aria-multiselectable="true" class="content-accordion" id="${
            this.state.accordionTitleForIds
        }" role="tablist">${output.join('')}</div>
            ...
            <div className="accordionater container-fluid">
                <div className="row">
                    <div className="col-md-12">
                        <h1>Accordionater</h1>
                        <p>
                            Welcome to Accordionater, the bootstrap accordion
                            generator for the Baltimore County sites.
                        </p>
                    </div>
                </div>
